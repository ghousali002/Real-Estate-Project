...
import styles from '../Listing/Avatar.module.css';
import Row from '../../ui/Row';
import { useBuyerUser } from './useBuyerUser';
...
export default function BuyerProfile() {
  const { data, isLoading, isError,  refetchUser } = useBuyerUser() ;
...
},
{
  ...
  return (
    <>
      <BuyerHeader/>
      <Row type="horizontal">
        <Heading as="head1" style={{ marginLeft: '20px' }}>My Profile</Heading>
      </Row>
        ...
        <ProfileCard>
          <div style={{ textAlign: 'center' }}>
            ...
            {Photo ? <img src={handleImageSrc(Photo)} /> : 
             <img src={publicUrl + "assets/img/default-user.jpg"}/> }
            </ImageContainer>
            {/* Profile Name */}
            <h6>{Name}</h6>
            {/* Username */}
        ...
        {reviews.map((review, index) => (
          <ReviewCard key={index}>
            <ReviewHeader>
              <div>
                <img
                  src={publicUrl + "assets/img/default-user.jpg"}
